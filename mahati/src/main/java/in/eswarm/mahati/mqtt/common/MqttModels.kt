...
data class MqttConnectionParams(
    ...
    // It's good practice to override equals for ByteArray if it's part of the primary constructor
    // to ensure proper comparison, though for simple use cases it might not be strictly necessary.
...
sealed interface MqttClientState {
    data object Disconnected : MqttClientState // Client is disconnected or connection attempt was cancelled.
    data object Connecting : MqttClientState   // Client is attempting to connect.
    data class Connected(val serverUri: String) : MqttClientState // Client is successfully connected.
    data class Error(val message: String, val cause: Throwable? = null) : MqttClientState // An error occurred.
}
